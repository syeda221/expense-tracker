<x-app-layout>
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('expenses.update', $expense) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" class="form-control @error('description') is-invalid @enderror" name="description" rows="2" required>{{ old('description', $expense->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input id="merchant" type="text" class="form-control @error('merchant') is-invalid @enderror" name="merchant" value="{{ old('merchant', $expense->merchant) }}">
                            @error('merchant')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-body">
                    <h6 class="card-title">AI Classification</h6>
                    @if ($expense->ai_confidence !== null)
                        <p class="mb-1"><strong>Confidence:</strong> {{ $expense->ai_confidence * 100 }}%</p>
                        <p class="mb-0"><strong>Recurring:</strong> {{ $expense->is_recurring ? 'Yes' : 'No' }}</p>
                    @else
                        <p class="text-muted small mb-0">AI classification will run after saving.</p>
                    @endif
                </div>
            </div>
        </div>
